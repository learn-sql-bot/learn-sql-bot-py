...
class DataProviderLocal(DataProvider):
    ...
    def _get_text_from_file(self, topic: str, code: str) -> str:
        ...
        task_md =  task_file.read()
        return task_md

    def get_exercise_by_topic_and_code(self, topic: str, code: str) -> Exercise:

        sql = self._get_sql_from_file(topic, code)
        text = self._get_text_from_file(topic, code)

        exercise: Exercise = Exercise(topic=topic, code=code,sql=sql, text=text)
